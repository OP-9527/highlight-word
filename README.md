...
English Word Highlighter 是一个 Chrome Manifest V3 扩展，用于在网页中高亮英文生词，并提供翻译、发音、已知单词管理、自定义词表高亮和跨设备同步能力。它适合英语阅读、词汇积累和按主题词表进行针对性学习。
...
- `manifest.json`：扩展配置和内容脚本注入规则。
- `background.js`：处理扩展图标点击、受限页面判断和外部翻译请求。
- `content/`：按功能拆分的内容脚本，依次加载配置状态、DOM 上下文、高亮、弹窗翻译、词表同步、生命周期、侧边栏、选词翻译和启动入口。
...
- Cambridge Dictionary：发音音频来源。
- Bing Dictionary：中文释义和补充词典内容。

翻译结果会在页面内缓存约 24 小时，最多缓存 500 条解析后的弹窗数据，以减少重复请求和页面内存占用。
